...
class PerritosAdapterConCheck(val context: Activity, val perritos: ArrayList<Perritos>) : RecyclerView.Adapter<PerritosAdapterConCheck.PerritosViewHolder>() {
    ...
    override fun onBindViewHolder(holder: PerritosViewHolder, position: Int) {
        ...
        holder.itemView.setOnClickListener{
            (context as SeleccionarMascotaCu).resetValue(prev)
            (context as SeleccionarMascotaCu).resetValue(positionPerritoSesion)
            prev = position
            holder.imageViewCheck.visibility = View.VISIBLE
